...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const after = core.getInput('after', { required: true });
            const releaseType = core.getInput('release_type', { required: true });
            core.setOutput('newer', (0, version_1.isNewerVersion)(before, after, releaseType));
        }
        catch (error) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.isNewerVersion = exports.parseReleaseType = void 0;
const semver_1 = __importDefault(__nccwpck_require__(1383));
const allowed_release_types = ['major', 'premajor', 'minor', 'preminor', 'patch', 'prepatch', 'prerelease'];
function parseReleaseType(str) {
    if (!allowed_release_types.includes(str)) {
        throw new Error(`Invalid release type: ${str}`);
    }
    return str; // type assertion
}
exports.parseReleaseType = parseReleaseType;
function isNewerVersion(_before, _after, _releaseType) {
    if (!_before) {
    ...
    const releaseType = parseReleaseType(_releaseType);
    const before = semver_1.default.clean(_before);
    if (!before) {
        throw new Error(`Cannot infer valid semver version from \`before\` input: ${_before}`);
    }
    const after = semver_1.default.clean(_after);
    if (!after) {
    ...
    const next = semver_1.default.inc(before, releaseType);
    return semver_1.default.gte(after, next);
}
